...
export async function createCustomers(formData: FormData) {
  ...
  const { nama, no_telp, pesanan, date, image_url } = CreateCustomers.parse({
  ...
  try {
    await sql`
        INSERT INTO customers (nama, no_telp, pesanan, date, image_url)
        VALUES (${nama}, ${no_telp}, ${pesanan}, ${date}, ${image_url})
      `;
  } catch (error) {
    return {
